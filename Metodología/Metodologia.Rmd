...
```{r tasa_rendimiento}
# Se crea un nuevo DataFrame eliminando las columnas 1, 4, 5 y 6 del DataFrame original de activos
df.act <- activos[, -c(1, 4, 5, 6)]
...
# Se calcula el rendimiento mensual del activo neto a valor de mercado
df.act$rend.vm <- c(NA, diff(df.act$activo_neto_vm) / head(df.act$activo_neto_vm, -1))

# Se calcula el rendimiento mensual del activo neto a valor contable
df.act$rend.vc <- c(NA, diff(df.act$activo_neto_vc) / head(df.act$activo_neto_vc, -1))
...
# Se calcula la tasa mensual del rendimiento del activo neto a valor de mercado
tasa.mensual.vm <- prod(1 + df.act$rend.vm)^(1 / length(df.act$rend.vm)) - 1

# Se anualiza la tasa mensual a valor de mercado
tasa.anual.vm <- (1 + tasa.mensual.vm)^12 - 1

# Se calcula la tasa mensual del rendimiento del activo neto a valor contable
tasa.mensual.vc <- prod(1 + df.act$rend.vc)^(1 / length(df.act$rend.vc)) - 1

# Se anualiza la tasa mensual a valor contable
tasa.anual.vc <- (1 + tasa.mensual.vc)^12 - 1
