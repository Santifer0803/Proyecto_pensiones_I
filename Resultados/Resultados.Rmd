...
# Se crea el vector con las tasas de acumulación
s <- ((1 + interes.real)^(0:(95 - 1)))
...
edades.completas <- outer(edades, 0:94, "+")
colnames(edades.completas) <- colnames(edades.h)

mort.completa.h <- matrix(0, nrow = length(edades), ncol = ncol(edades.h))
mort.completa.m <- matrix(0, nrow = length(edades), ncol = ncol(edades.m))

colnames(mort.completa.h) <- colnames(edades.h)
colnames(mort.completa.m) <- colnames(edades.m)

for (i in 1:ncol(mort.completa.h)) {
  mort.completa.h[, i] <-
    mortalidad[sex == 1 &
                 year == as.double(colnames(mort.h)[i])][.(edad = edades.completas[, i]), 
                                                         on = .(edad), qx]
  mort.completa.m[, i] <-
    mortalidad[sex == 2 &
                 year == as.double(colnames(mort.m)[i])][.(edad = edades.completas[, i]), 
                                                         on = .(edad), qx]
}
...
Se procede a calcular los beneficicos para las 3 modalidades de pensión (vejez, invalidez, sucesión)

Primero los parámetros generales:
...
### Pensiones por vejez por año y sexo

```{r pensiones_vejez}
matriz.vejez.h <- estados.h[filas.vejez.h, , drop = FALSE]
matriz.vejez.m <- estados.m[filas.vejez.m, , drop = FALSE]

edades.vejez.h <- edades.h[filas.vejez.h, , drop = FALSE]
edades.vejez.m <- edades.m[filas.vejez.m, , drop = FALSE]

indices.1h <- apply(matriz.vejez.h, 1, function(fila) {
...
indices.1m <- apply(matriz.vejez.m, 1, function(fila) {
  which(fila == "Retirado")[1]
})
...
for (i in 1:nrow(matriz.vejez.h)) {
  ...
  matriz.vejez.h[i, indices.1h[i]:(indices.2h[i] - 1)] <- 1
  
  edad <- edades.vejez.h[i, indices.2h[i]]
  
  qx.conyuge <- mort.completa.m[(edad - indices.2h[i] + 2), 
                                indices.2h[i]:ncol(mort.completa.m)]
  
  vect.aleatorio <- runif(length(qx.conyuge))
  
  estados.conyuge <- qx.conyuge > vect.aleatorio
  
  indice.3 <- which(estados.conyuge == TRUE)[1] - 1
  
  matriz.vejez.h[i, indices.2h[i]:(indices.2h[i] + indice.3)] <- 0.7
...
for (i in 1:nrow(matriz.vejez.m)) {
  ...
  matriz.vejez.m[i, indices.1m[i]:(indices.2m[i] - 1)] <- 1
  
  edad <- edades.vejez.m[i, indices.2m[i]]
  
  qx.conyuge <- mort.completa.h[(edad - indices.2h[i] + 2), 
                                indices.2m[i]:ncol(mort.completa.h)]
  ...
  matriz.vejez.m[i, indices.2m[i]:(indices.2m[i] + indice.3)] <- 0.7
  
}

matriz.vejez.h <- matriz.vejez.h * outer(pension.vejez.h, s, `*`)
matriz.vejez.m <- matriz.vejez.m * outer(pension.vejez.m, s, `*`)

colnames(matriz.vejez.h) <- colnames(edades.h)
colnames(matriz.vejez.m) <- colnames(edades.m)
```

### Pensiones por invalidez por año y sexo

```{r pensiones_invalidez}
edades.inv.h <- edades.h[filas.inv.h, , drop = FALSE]
edades.inv.m <- edades.m[filas.inv.m, , drop = FALSE]

matriz.inv.h <- estados.h[filas.inv.h, , drop = FALSE]
matriz.inv.m <- estados.m[filas.inv.m, , drop = FALSE]

indices.1h <- apply(matriz.inv.h, 1, function(fila) {
  which(fila == "Invalido")[1]
})
...
indices.1m <- apply(matriz.inv.m, 1, function(fila) {
  which(fila == "Invalido")[1]
})
...
for (i in 1:nrow(matriz.inv.h)) {
  ...
  matriz.inv.h[i, indices.1h[i]:(indices.2h[i] - 1)] <- 1
  
  edad <- edades.inv.h[i, indices.2h[i]]
  ...
  if (edad >= 50) {
    ...
    matriz.inv.h[i, indices.2h[i]:indice.3] <- ifelse(edad >= 60, 0.7, 0.6)
    
  }else{
    
    matriz.inv.h[i, indices.2h[i]:indice.3] <- 0.5
    
    qx.hijo <- mort.hijos[(edad - 25 - indices.2h[i] + 2), 
                          indices.2h[i]:ncol(mort.hijos)]
    
    vect.aleatorio <- runif(length(qx.hijo))
    
    estados.hijo <- qx.hijo > vect.aleatorio
    
    indice.4 <- which(estados.hijo == TRUE)[1] - 1
    
    indice.4 <- min(indice.4, (49 - edad))
    
    matriz.inv.h[i, indices.2h[i]:indice.4] <- 
      matriz.inv.h[i, indices.2h[i]:indice.4] + 0.3
...
for (i in 1:nrow(matriz.inv.m)) {
  ...
  matriz.inv.m[i, indices.1m[i]:(indices.2m[i] - 1)] <- 1
  
  edad <- edades.inv.m[i, indices.2m[i]]
  
  qx.conyuge <- mort.completa.h[(edad - indices.2m[i] + 2), 
                                indices.2m[i]:ncol(mort.completa.h)]
  ...
  if (edad >= 50) {
    ...
    matriz.inv.m[i, indices.2m[i]:indice.3] <- ifelse(edad >= 60, 0.7, 0.6)
    
  }else{
    
    matriz.inv.m[i, indices.2m[i]:indice.3] <- 0.5
    
    qx.hijo <- mort.hijos[(edad - 25 - indices.2m[i] + 2), 
                          indices.2m[i]:ncol(mort.hijos)]
    ...
    matriz.inv.m[i, indices.2m[i]:indice.4] <- 
      matriz.inv.m[i, indices.2m[i]:indice.4] + 0.3
    
  }
  
}

matriz.inv.h <- matriz.inv.h * outer(pension.invalidez.h, s, `*`)
matriz.inv.m <- matriz.inv.m * outer(pension.invalidez.m, s, `*`)
```
