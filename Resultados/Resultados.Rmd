...
```{r}
cols_anios <- as.character(1995:2024)

data_binaria <- data_anual_sumado

data_binaria[cols_anios] <- lapply(data_binaria[cols_anios], function(x) ifelse(x == 0, 0, 1))
```
...
probabilidades_anuales <- sapply(data_binaria[cols_anios], function(columna) {
  mean(columna == 1)  
})

probabilidades_df <- data.frame(
  anio = as.integer(names(probabilidades_anuales)),
  probabilidad_activo = probabilidades_anuales
)
```


```{r}
proba.activo <- probabilidades_df %>%
  filter(anio == 2023) %>%
  pull(probabilidad_activo)
proba.activo
```

```{r}
sum(df.anualizado == 0, na.rm = TRUE)
dim(df.anualizado)
```
```{r}
# Se crea la matriz de hombres
inactivo.h <- matrix(0.4148383, nrow = 2595, ncol = 95)

# Se crea la matriz de mujeres
inactivo.m <- matrix(0.4148383, nrow = 2601, ncol = 95)
```
...
```{r}
postergacion.simulada.h <- matriz.aleatoria.h2 < df.probabilidad.postergacion.h
...
```{r}
postergacion.simulada.m <- matriz.aleatoria.m2 < df.probabilidad.postergacion.m
