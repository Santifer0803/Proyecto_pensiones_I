...
matriz.vp <-
  matrix(((1 + tasa.mensual) ^ -((potencia.mensual - 1))), nrow(cotizantes), 
         length(potencia.mensual), byrow = TRUE)

# Se repite el proceso para salarios
potencia.mensual <-
  rep(seq(1:(ncol(edades) * 12)))
matriz.vp.salarios <-
  matrix(((1 + tasa.mensual) ^ -((potencia.mensual - 1))), nrow(cotizantes), 
...
while (abs(prom.despues - prom.antes) >= 0.00001) {
  ...
  matriz.aleatoria.h <-
    matrix(runif(nrow(cotizantes.h) * ncol(mort.h)),
           nrow(cotizantes.h), ncol(mort.h))
  
  matriz.aleatoria.m <-
    matrix(runif(nrow(cotizantes.m) * ncol(mort.m)),
  ...
  salarios.aux.h <-
    do.call(rbind, lapply(1:nrow(salario.estoc.h), function(i) {
  ...
  salarios.aux.m <-
    do.call(rbind, lapply(1:nrow(salario.estoc.m), function(i) {
  ...
  beneficio.base.h <-
    rowSums(salarios.sucesion.h) / rowSums(salarios.top.h[which(salida.h == "Fallecido"), ] > 9999)
  beneficio.base.m <-
    rowSums(salarios.sucesion.m) / rowSums(salarios.top.m[which(salida.m == "Fallecido"), ] > 9999)
  ...
  meses.post.h <-
    12 * rowSums(postergacion.simulada.h[filas.vejez.h,])
  meses.post.m <-
    12 * rowSums(postergacion.simulada.m[filas.vejez.m,])
  ...
  pension.vejez.h <-
    salario.referencia.h * pmin(tasa.vejez.h, tasa.maxima)
  pension.vejez.m <-
    salario.referencia.m * pmin(tasa.vejez.m, tasa.maxima)
  ...
  pension.invalidez.h <-
    salario.referencia.inv.h * pmin(tasa.invalidez.h, tasa.maxima)
  pension.invalidez.m <-
    salario.referencia.inv.m * pmin(tasa.invalidez.m, tasa.maxima)
  ...
  matriz.vejez.h <-
    matrix(as.numeric(0), nrow(matriz.vejez.h), ncol(matriz.vejez.h))
  matriz.vejez.m <-
    matrix(as.numeric(0), nrow(matriz.vejez.m), ncol(matriz.vejez.m))
  ...
  for (i in 1:nrow(matriz.vejez.h)) {
    ...
    n <-
      min(nrow(mort.completa.m) - edad,
          ncol(mort.completa.m) - indice + 1)
    
    # Se extraen las probabilidades de muerte del cónyuge (mujer) por edad y año
    qx.conyuge <-
      mort.completa.h[cbind(min((edad + 1), 116):(edad + n), indice:(indice + n - 1))]
  ...
  for (i in 1:nrow(matriz.vejez.m)) {
    ...
    n <-
      min(nrow(mort.completa.h) - edad,
          ncol(mort.completa.h) - indice + 1)
  ...
  monto.vejez.h <-
    matrix(pension.vejez.h,
           nrow(matriz.vejez.h),
           ncol(matriz.vejez.h))
  monto.vejez.m <-
    matrix(pension.vejez.m,
           nrow(matriz.vejez.m),
           ncol(matriz.vejez.m))
  ...
  matriz.inv.h <-
    matrix(as.numeric(0), nrow(matriz.inv.h), ncol(matriz.inv.h))
  matriz.inv.m <-
    matrix(as.numeric(0), nrow(matriz.inv.m), ncol(matriz.inv.m))
  
  # Llenamos la matriz de hombres
  for (i in 1:nrow(matriz.inv.h)) {
    indice <- indices.2h[i]  # Primer año de fallecimiento
    matriz.inv.h[i, restantes.inv.h[i]:(indice - 1)] <-
      1  # Periodo de invalidez antes de fallecer
    ...
    estados.conyuge <- qx.conyuge >= vect.aleatorio
    indice.3 <-
      which(estados.conyuge == TRUE)[1] - 1  # Tiempo que sobrevive el cónyuge
    ...
    if (edad >= 60) {
      ...
      n <-
        min(nrow(mort.hijos) - edad + 25, ncol(mort.hijos) - indice + 1)
      
      qx.hijo <-
        mort.hijos[cbind((edad - 24):(edad - 25 + n), indice:(indice + n - 1))]
      ...
      indice.4 <-
        min(indice.4, (49 - edad), na.rm = TRUE)  # Máximo hasta que el hijo cumpla 25
  ...
  monto.inv.h <-
    matrix(pension.invalidez.h,
           nrow(matriz.inv.h),
           ncol(matriz.inv.h))
  monto.inv.m <-
    matrix(pension.invalidez.m,
           nrow(matriz.inv.m),
           ncol(matriz.inv.m))
  ...
  matriz.suc.h <-
    matrix(as.numeric(0), nrow(estados.h[filas.muerte.h,]),
           ncol(estados.h[filas.muerte.h,]))
  
  matriz.suc.m <-
    matrix(as.numeric(0), nrow(estados.m[filas.muerte.m,]),
  ...
  for (i in 1:nrow(matriz.suc.h)) {
    ...
    n <-
      min(nrow(mort.completa.m) - edad, ncol(mort.completa.m) - col + 1)
    
    # Se obtienen las probabilidades de fallecimiento del cónyuge
    qx.conyuge <-
      mort.completa.h[cbind(min((edad + 1), 116):(edad + n), col:(col + n - 1))]
    ...
    if (edad >= 60) {
      ...
      # Simulamos la mortalidad del hijo
      n <-
        min(nrow(mort.hijos) - edad + 25, ncol(mort.hijos) - col + 1)
      qx.hijo <-
        mort.hijos[cbind((edad - 24):(edad - 25 + n), col:(col + n - 1))]
      vect.aleatorio <- c(1, runif(length(qx.hijo) - 1))
      estados.hijo <- qx.hijo >= vect.aleatorio
      indice.4 <- which(estados.hijo == TRUE)[1] - 1
      indice.4 <- min(indice.4, (49 - edad), na.rm = TRUE)
  ...
  for (i in 1:nrow(matriz.suc.m)) {
    ...
    n <-
      min(nrow(mort.completa.h) - edad, ncol(mort.completa.h) - col + 1)
  ...
  sucesion.aux.h <-
    ifelse(tiene.derecho.sucesion.h, beneficio.base.h, 0)
  sucesion.aux.m <-
    ifelse(tiene.derecho.sucesion.m, beneficio.base.m, 0)
  
  # Se crean matrices con los montos por sucesión
  monto.suc.h <-
    matrix(sucesion.aux.h, nrow(matriz.suc.h), ncol(matriz.suc.h))
  monto.suc.m <-
    matrix(sucesion.aux.m, nrow(matriz.suc.m), ncol(matriz.suc.m))
  ...
  # Inicializamos las matrices de beneficio con ceros
  matriz.vejini.h <-
    matrix(as.numeric(0), nrow(pensini.vej.h), ncol(edades))
  matriz.vejini.m <-
    matrix(as.numeric(0), nrow(pensini.vej.m), ncol(edades))
  ...
  # Inicializamos las matrices de beneficio con ceros
  matriz.invini.h <-
    matrix(as.numeric(0), nrow(pensini.inv.h), ncol(edades))
  matriz.invini.m <-
    matrix(as.numeric(0), nrow(pensini.inv.m), ncol(edades))
  ...
  # Inicializamos las matrices de beneficio con ceros
  matriz.suc.ch <-
    matrix(as.numeric(0), nrow(pension.suc.ch), ncol(edades))
  matriz.suc.cm <-
    matrix(as.numeric(0), nrow(pension.suc.cm), ncol(edades))
  ...
  # Inicializamos las matrices de beneficio con ceros
  matriz.suc.hh <-
    matrix(as.numeric(0), nrow(pension.suc.hh), ncol(edades))
  matriz.suc.hm <-
    matrix(as.numeric(0), nrow(pension.suc.hm), ncol(edades))
  ...
  for (i in 1:length(matrices.pension)) {
    ...
    matrices.vp[[i]] <-
      matrices.pension[[i]] * matriz.vp[1:nrow(matrices.pension[[i]]),]
  ...
  salarios.finales <- rbind(salarios.aux.h,
                            salarios.aux.m)
  
  # Se calcula la masa salarial
  masa.salarial <- sum(salarios.finales * matriz.vp.salarios)
  ...
  for (i in 1:ncol(edades)) {
    # Cotizaciones de los trabajadores
    cot.trab <- sum(salarios.finales[, (12 * i - 11):(12 * i)]) * 0.15
    
    # Cotizaciones de los pensionados
    cot.pens <- Reduce("+", sapply(matrices.pension,
                                   function(mat)
                                     sum(mat[, (13 * i - 12):(13 * i)][mat[, (13 * i - 12):(13 * i)] > 2000000])))
    
    cot.pens <- 0.05 * cot.pens
    
    # Cotizaciones totales
    cotizaciones <- c(cotizaciones, (cot.trab + cot.pens))
    
    # Costo de las pensiones
    costo.pens <-
      Reduce("+", sapply(matrices.pension, function(mat)
        sum(mat[, (13 * i - 12):(13 * i)])))
    ...
    # Rendimientos
    rends <-
      c(rends, ifelse((reserva[i] + (cotizaciones[i] - total.egresos[i]) / 2) * interes.real > 0,
                      (reserva[i] + (cotizaciones[i] - total.egresos[i]) / 2) * interes.real,
                      0
      ))
    ...
    reserva <-
      c(reserva, (reserva[i] + total.ingresos - total.egresos[i]))
  }
  ...
  razon.solvencia <-
    append(razon.solvencia, (total.activo / total.pasivo))
  annos.crit <- append(annos.crit, list(annos))
  balance <- append(balance, list(c(total.activo, total.pasivo)))
  prima.media <-
    append(prima.media, ((total.pasivo - reserva.ini) / masa.salarial))
  ...
  beneficio.devengado.vejez <-
    append(beneficio.devengado.vejez, (sum(ben.dev.vejez.h) + sum(ben.dev.vejez.m)))
  beneficio.devengado.invalidez <-
    append(beneficio.devengado.invalidez, (sum(ben.dev.inv.h) + sum(ben.dev.inv.m)))
  beneficio.devengado.sucesion <-
    append(beneficio.devengado.sucesion, (sum(ben.dev.suc.h) + sum(ben.dev.suc.m)))
...
```{r metricas_base}
# Inicializamos la reserva inicial en el 2025 y los vectores del flujo
reserva <- c(reserva.ini)
salarios <- c()
cotizaciones <- c()
costo.vejez <- c()
...
# Inicializamos los vectores para las métricas
tasa.ingresos <- c()
prima.reparto <- c()
razon.fondo.cont <- c()
...
for(i in 1:ncol(edades)){
  ...
  # Masa salarial
  salarios <- c(salarios, sum(salarios.finales[, (12*i - 11):(12*i)]))
  
  # Cotizaciones de los trabajadores
  cot.trab <- salarios[i] * 0.15
  
  # Cotizaciones de los pensionados
  cot.pens <- Reduce("+", sapply(matrices.pension, 
                     function(mat) sum(mat[, (13*i - 12):(13*i)][mat[, (13*i - 12):(13*i)] > 2000000])))
  ...
  # Costo de las pensiones por vejez
  vejez <- Reduce("+", sapply(matrices.pension[1:2], function(mat) sum(mat[, (13*i - 12):(13*i)]))) +
    Reduce("+", sapply(matrices.pension[7:8], function(mat) sum(mat[, (13*i - 12):(13*i)])))
  
  costo.vejez <- c(costo.vejez, vejez)
  
  # Costo de las pensiones por invalidez
  inv <- Reduce("+", sapply(matrices.pension[3:4], function(mat) sum(mat[, (13*i - 12):(13*i)]))) + 
    Reduce("+", sapply(matrices.pension[9:10], function(mat) sum(mat[, (13*i - 12):(13*i)])))
  
  costo.inv <- c(costo.inv, inv)
  
  # Costo de las pensiones por sucesión
  suc <- Reduce("+", sapply(matrices.pension[5:6], function(mat) sum(mat[, (13*i - 12):(13*i)]))) + 
    Reduce("+", sapply(matrices.pension[11:12], function(mat) sum(mat[, (13*i - 12):(13*i)])))
  
  costo.suc <- c(costo.suc, suc)
  
  # Costo total de las pensiones
  costo.pens <- costo.vejez[i] + costo.inv[i] + costo.suc[i]
  
  # Gasto del SEM
  sem <- c(sem, costo.pens * (12/13) * 0.085)
  
  # Egresos totales
  total.egresos <- c(total.egresos, (costo.pens + sem[i]))
  ...
  # Ingresos totales
  total.ingresos <- c(total.ingresos, (cotizaciones[i] + rends[i]))
  
  # Guardamos las métricas
  tasa.ingresos <- c(tasa.ingresos, 
                     ifelse(salarios[i] > 0, (total.ingresos[i] / salarios[i]), NA))
  
  prima.reparto <- c(prima.reparto, 
                     ifelse(salarios[i] > 0, (total.egresos[i] / salarios[i]), NA))
  
  razon.fondo.cont <- c(razon.fondo.cont, 
                        ifelse(total.egresos[i] > 0, (reserva[i] / total.egresos[i]), NA))
  
  # Actualizamos la reserva inicial para el siguiente año
  reserva <- c(reserva, (reserva[i] + total.ingresos[i] - total.egresos[i]))
}

# DataFrame con el flujo
flujo <- data.frame(
  
  anno = ((1:ncol(edades)) + 2024),
  
  reserva.ini = reserva[1:(length(reserva) - 1)] / 1000000,
  
  salarios = salarios / 1000000,
  
  cotizaciones = cotizaciones / 1000000,
  
  rendimientos = rends / 1000000,
  
  total.ingresos = total.ingresos / 1000000,
  
  costo.vejez = costo.vejez / 1000000,
  
  costo.inv = costo.inv / 1000000,
  
  costo.suc = costo.suc / 1000000,
  
  gasto.sem = sem / 1000000,
  
  total.egresos = total.egresos / 1000000,
  
  reserva.fin = reserva[2:length(reserva)] / 1000000
  
)

# Formato latex
print(xtable(flujo), type = "latex")
```

# Resultados base

## Proyección demográfica representativa

Se inicia mostrando una proyección demográfica anual, de las personas que están activas, inactivas y pensionadas. Ya que se tiene un modelo estocástico, estas cambiarán en cada escenario, por lo que se muestra de forma representativa y no tiene que seguir necesariamente el comportamiento general del fondo.

```{r proyeccion_demografica_base}
# Dataframe con los datos por año
df.proyeccion.demografica <- data.frame(
  annos = 2025:2119,
  activos = 0,
  inactivos = 0,
  pensionados = 0
...
# Cantidad de activos por año
df.proyeccion.demografica[, 2] <-
  colSums(estados.h == "Activo") + colSums(estados.h == "Posterga") + colSums(estados.m == "Activo") + colSums(estados.m == "Posterga")

# Cantidad de inactivos por año
df.proyeccion.demografica[, 3] <-
  colSums(estados.h == "Inactivo") + colSums(estados.m == "Inactivo")

# Cantidad de pensionados por año
df.proyeccion.demografica[, 4] <-
  (
    colSums(matriz.vejez.h > 0) + colSums(matriz.vejez.m > 0) + colSums(matriz.inv.h > 0) + colSums(matriz.inv.m > 0) + colSums(matriz.suc.h > 0) + colSums(matriz.suc.m > 0)
  )

# Se grafican las curvas correspondientes
ggplot(df.proyeccion.demografica, aes(x = annos)) +
  geom_line(aes(y = activos, color = "Activos"), linewidth = 1) +
  geom_line(aes(y = inactivos, color = "Inactivos"), linewidth = 1) +
  geom_line(aes(y = pensionados, color = "Pensionados"), linewidth = 1) +
  scale_color_manual(
    name = "Estado",
    values = c(
      "Activos" = "#2E7D32",
  ...
  labs(x = "Años", y = "Cantidad de personas") +
  theme_minimal()
```
...
# Variable de curva modificada
curva.mod <- curva.general %>% filter(proyeccion.edad < 116)

ggplot(curva.mod, aes(x = proyeccion.edad, y = salario.promedio)) +
  geom_line(color = "darkgreen", linewidth = 1.2) +
  labs(x = "Edad proyectada",
       y = "Salario promedio") +
  theme_minimal()
```
...
ggplot(curva.mod, aes(x = proyeccion.edad, y = salario.promedio)) +
  ...
  geom_smooth(method = "loess", formula = y ~ x, color = "blue", linewidth = 1.2, se = FALSE) +
  labs(x = "Edad proyectada",
       y = "Salario promedio") +
  theme_minimal()
```
...
ggplot(data.rs, aes(x = rs)) +
  geom_density(aes(y = ..scaled..), color = "#A34022", linewidth = 1.2) +
  geom_vline(
    xintercept = prom.rs,
    color = "#F59A3A",
    linetype = "dashed",
...
# Se quitan los nulos
data.rc <- as.data.frame(data.rc$rc[which(!is.na(data.rc$rc))])

# Se agrega el índice
data.rc$indice <- 1:nrow(data.rc)

# Se cambia el nombre de la columna
colnames(data.rc) <- c("rc", "id")

# Gráfico de las razones del fondo de contingencia
ggplot(data.rc, aes(x = id, y = rc / 1000000)) +
  geom_line(color = "#A34022", linewidth = 1) +
  labs(x = "Año", y = "Razón del fondo de contingencia (Millones)") +
  theme_minimal()
```

Una vez más, se aprecia que la reserva es muy grande en comparación con los egresos, lo cual produce resultados muy grandes para esta métrica.
...
# Se quitan los nulos
data.ti <- as.data.frame(data.ti$ti[which(!is.na(data.ti$ti))])

# Se agrega el índice
data.ti$indice <- 1:nrow(data.ti)

# Se cambia el nombre de la columna
colnames(data.ti) <- c("ti", "id")

# Gráfico de las tasas de ingresos
ggplot(data.ti, aes(x = id, y = ti)) +
  geom_line(color = "#A34022", linewidth = 1) +
  labs(x = "Año", y = "Tasa de ingresos") +
  theme_minimal()
```
...
# Se quitan los nulos
data.pr <- as.data.frame(data.pr$pr[which(!is.na(data.pr$pr))])

# Se agrega el índice
data.pr$indice <- 1:nrow(data.pr)

# Se cambia el nombre de la columna
colnames(data.pr) <- c("pr", "id")

# Gráfico de las primas de reparto
ggplot(data.pr, aes(x = id, y = pr)) +
  geom_line(color = "#A34022", linewidth = 1) +
  labs(x = "Año", y = "Prima de reparto") +
  theme_minimal()
```
...
# Gráfico de pasivos devengados
ggplot(beneficio.devengado.final, aes(x = categoria, y = (ben.dev / 1000000000))) +
  geom_bar(stat = "identity", fill = "#A34022") +
    labs(x = "Categoría del pasivo devengado", y = "Valor (Miles de millones de colones)") +
  theme_minimal()
```

Se grafican los activos y pasivos devengados, de forma agregada.

```{r balance_devengado}
# Beneficio devengado con
beneficio.devengado.agregado <-
  data.frame(ben.dev = c(rowMeans(matriz.balance)[1], sum(beneficio.devengado.final[, 1])),
             categoria = c("Activo", "Pasivo"))

# Gráfico de beneficios devengados
ggplot(beneficio.devengado.agregado, aes(x = categoria, y = (ben.dev / 1000000000))) +
  geom_bar(stat = "identity", fill = "#A34022") +
  labs(x = "Categoría del devengo", y = "Valor (Miles de millones de colones)") +
  theme_minimal()

# Razón de solvencia devengada
rowMeans(matriz.balance)[1] / sum(beneficio.devengado.final[, 1])
```
...
for (i in 1:nrow(matriz.vejez.h)) {
  ...
  # Se extraen las probabilidades de muerte del cónyuge (mujer) por edad y año
  qx.conyuge <- mort.completa.h[cbind(min((edad + 1), 116):(edad + n), indice:(indice + n - 1))]
...
for(i in 1:nrow(matriz.suc.h)){
  ...
  # Se obtienen las probabilidades de fallecimiento del cónyuge
  qx.conyuge <- mort.completa.h[cbind(min((edad + 1), 116):(edad + n), col:(col + n - 1))]
...
for(i in 1:ncol(edades)){
  ...
  # Guardamos la reserva final (reserva inicial del siguiente año)
  reserva <- c(reserva, (reserva[i] + total.ingresos - total.egresos[i]))
}
...
for(i in 1:ncol(edades)){
  ...
  # Masa salarial
  ms <- sum(salarios.finales[, (12*i - 11):(12*i)])
  ...
  # Cotizaciones totales
  cotizaciones <- cot.trab + cot.pens
  ...
  razon.fondo.cont <- c(razon.fondo.cont, ifelse(total.egresos > 0, (reserva / total.egresos), NA))
  
  # Actualizamos la reserva inicial para el siguiente año
  reserva <- reserva + total.ingresos - total.egresos
...
## Resultados de la disminución de mortalidad
...
# Eliminación del retiro anticipado

Se debe proceder a modificar la mortalidad en las variables correspondientes, para devolverla a la normalidad.
...
En este caso, se modifican las variables que permiten el retiro anticipado de las mujeres.

```{r retiro_anticipado_as3}
# Se modifican las probabilidades de postergación fijas, de forma que empiece en 65 años para las mujeres también
df.probabilidad.postergacion.h <- ifelse(edades.h >= 65, 0.1, 0)
...
estados.m <- ifelse(lista.estados.m[[2]] == 1,
         "Fallecido",
         ifelse(
           lista.estados.m[[1]] == 1,
           "Invalido",
           ...
           "Posterga",
           ifelse(((((cotizaciones.acum.m > 179) * (edades.m > 64)
           ) == 0) * (actividad.m == 1)) == 1,
           "Activo",
           ifelse(((((cotizaciones.acum.m > 179) * (edades.m > 64)
           ) == 0) * (actividad.m == 0)) == 1, "Inactivo", ifelse(
             edades.m > 64,
             "Retirado",
             ifelse(actividad.m == 1, "Activo", "Inactivo")
           ))
...
## Resultados de la eliminación del retiro anticipado
...
En este caso, se aumentarán todas las edades de retiro en 5 años. Estas serían la edad de retiro por vejez y las edades de retiro anticipado de las mujeres

```{r retiro_anticipado_as4}
# Se modifican las probabilidades de postergación fijas, de forma que empiece en 65 años para las mujeres también
df.probabilidad.postergacion.h <- ifelse(edades.h > 69, 0.1, 0)
df.probabilidad.postergacion.m <- ifelse(edades.m > 67, 0.1, 0)
```

Posteriormente, se tiene un modelo idéntico. Iniciamos con las listas.

```{r listas_iniciales_as4}
razon.solvencia <- list()
annos.crit <- list()
balance <- list()
prima.media <- list()
...
estados.h <-
  ifelse(lista.estados.h[[2]] == 1,
         ...
         ifelse(
           lista.estados.h[[1]] == 1,
           "Invalido",
           ifelse(((postergacion.simulada.h == 1) * (cotizaciones.acum.h > 179) * (edades.h > 69)
           ) == 1,
           "Posterga",
           ifelse(((((cotizaciones.acum.h > 179) * (edades.h > 69)
           ) == 0) * (actividad.h == 1)) == 1,
           "Activo",
           ifelse(((((cotizaciones.acum.h > 179) * (edades.h > 69)
           ) == 0) * (actividad.h == 0)) == 1, "Inactivo", ifelse(
...
estados.m <-
  ifelse(lista.estados.m[[2]] == 1,
         ...
         ifelse(
           lista.estados.m[[1]] == 1,
           "Invalido",
           ifelse(((postergacion.simulada.m == 1) * (cotizaciones.acum.m > 404) * (edades.m == 68)
           ) == 1,
           "Posterga",
           ifelse(((postergacion.simulada.m == 1) * (cotizaciones.acum.m > 356) * (edades.m == 69)
           ) == 1,
           "Posterga",
           ifelse(((postergacion.simulada.m == 1) * (cotizaciones.acum.m > 179) * (edades.m > 69)
           ) == 1,
           "Posterga",
           ifelse(
             ((edades.m == 68) * (cotizaciones.acum.m > 404)) == 1,
             "Retirado",
             ifelse(((edades.m == 69) * (cotizaciones.acum.m > 356)
             ) == 1, "Retirado", ifelse(((edades.m > 69) * (cotizaciones.acum.m > 179)) == 1,
                                        "Retirado",
                                        ifelse(actividad.m == 1, "Activo", "Inactivo")
             ))
           ))
...
## Resultados del aumento de la edad de retiro
