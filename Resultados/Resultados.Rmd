...
potencia.mensual <-
  rep(seq(1:(95 * 12)), times = ifelse((seq(1:(
    95 * 12
  )) %% 12) == 0, 2, 1))

# Se obtiene el valor presente de cada periodo proyectado
matriz.vp <-
  matrix(((1 + tasa.mensual) ^ -(potencia.mensual / 12)), ncol = length(potencia.mensual), nrow = nrow(cotizantes), byrow = TRUE)
```
...
# Filtramos los pensionados hombres con pensión por invalidez o vejez
pension.vejinv.h <- pensionados.h[pensionados.h$cod_tipo_pension %in% c("Invalidez", "Vejez"), ]

# Determinamos el año en el que empezó a regir cada pensión
annos <- year(pension.vejinv.h$rige_de_la_pension) - 1959

# Edad de cada pensionado
edad.vejinv.h <- 2025 - year(pension.vejinv.h$fec_nac)

# Acumulamos el monto de pensión por la inflación correspondiente
pension.vejinv.h <- pension.vejinv.h$monto * inflacion.cr$factor_acumulado[annos]
pension.vejinv.h <- matrix(pension.vejinv.h, length(pension.vejinv.h), ncol(edades))

# Se repite el proceso para las mujeres
pension.vejinv.m <- pensionados.m[pensionados.m$cod_tipo_pension %in% c("Invalidez", "Vejez"), ]
annos <- year(pension.vejinv.m$rige_de_la_pension) - 1959
edad.vejinv.m <- 2025 - year(pension.vejinv.m$fec_nac)
pension.vejinv.m <- pension.vejinv.m$monto * inflacion.cr$factor_acumulado[annos]
pension.vejinv.m <- matrix(pension.vejinv.m, length(pension.vejinv.m), ncol(edades))

# Filtramos los pensionados hombres cónyuges con pensión por sucesión
...
# Inicializamos las matrices de beneficio con ceros
matriz.vejinv.h <- matrix(as.numeric(0), nrow(pension.vejinv.h), ncol(edades))
matriz.vejinv.m <- matrix(as.numeric(0), nrow(pension.vejinv.m), ncol(edades))

# Construimos la matriz de beneficios para hombres similar a los otros casos
for (i in 1:nrow(matriz.vejinv.h)) {
  
  edad <- edad.vejinv.h[i]
  ...
  edad <- edad.vejinv.h[i] + indice
  
  matriz.vejinv.h[i, 1:(indice - 1)] <- 1 
  ...
  if(edad >= 60){
    ...
    matriz.vejinv.h[i, indice:(indice + indice.3)] <- 0.7
    
  } else if(edad >= 50) {
    
    matriz.vejinv.h[i, indice:(indice + indice.3)] <- 0.6
    
    if((edad + indice.3) >= 60){
      
      matriz.vejinv.h[i, (indice + 60 - edad):(indice + indice.3)] <- 
        matriz.vejinv.h[i, (indice + 60 - edad):(indice + indice.3)] + 0.1
      
    }
  } else if(edad >= 25){
    
    matriz.vejinv.h[i, indice:(indice + indice.3)] <- 0.5
    
    if((edad + indice.3) >= 50 & (edad + indice.3) < 60){
      
      matriz.vejinv.h[i, (indice + 50 - edad):(indice + indice.3)] <- 
        matriz.vejinv.h[i, (indice + 50 - edad):(indice + indice.3)] + 0.1
      
    } else if((edad + indice.3) >= 60){
      
      matriz.vejinv.h[i, (indice + 50 - edad):(indice + 59 - edad)] <- 
        matriz.vejinv.h[i, (indice + 50 - edad):(indice + 59 - edad)] + 0.1
    ...
    matriz.vejinv.h[i, indice:(indice + indice.4)] <- 
      matriz.vejinv.h[i, indice:(indice + indice.4)] + 0.3
...
# Caso para mujeres
for (i in 1:nrow(matriz.vejinv.m)) {
  
  edad <- edad.vejinv.m[i]
  ...
  matriz.vejinv.m[i, 1:(indice - 1)] <- 1 
  ...
  if(edad >= 60){
    ...
    matriz.vejinv.m[i, indice:(indice + indice.3)] <- 0.7
    
  } else if(edad >= 50){
    
    matriz.vejinv.m[i, indice:(indice + indice.3)] <- 0.6
    
    if((edad + indice.3) >= 60){
      
      matriz.vejinv.m[i, (indice + 60 - edad):(indice + indice.3)] <- 
        matriz.vejinv.m[i, (indice + 60 - edad):(indice + indice.3)] + 0.1
      
    }
  } else if(edad >= 25){
    
    matriz.vejinv.m[i, indice:(indice + indice.3)] <- 0.5
    
    if((edad + indice.3) >= 50 & (edad + indice.3) < 60){
      
      matriz.vejinv.m[i, (indice + 50 - edad):(indice + indice.3)] <- 
        matriz.vejinv.m[i, (indice + 50 - edad):(indice + indice.3)] + 0.1
      
    } else if((edad + indice.3) >= 60){
      
      matriz.vejinv.m[i, (indice + 50 - edad):(indice + 59 - edad)] <- 
        matriz.vejinv.m[i, (indice + 50 - edad):(indice + 59 - edad)] + 0.1
    ...
    matriz.vejinv.m[i, indice:(indice + indice.4)] <- 
      matriz.vejinv.m[i, indice:(indice + indice.4)] + 0.3
...
matriz.vejinv.h <- matriz.vejinv.h * pension.vejinv.h
matriz.vejinv.m <- matriz.vejinv.m * pension.vejinv.m

# Se impone el tope máximo de 3.5 millones a los beneficios
matriz.vejinv.h <- pmin(matriz.vejinv.h, 3500000)
matriz.vejinv.m <- pmin(matriz.vejinv.m, 3500000)
...
rm(qx.conyuge, vect.aleatorio, estados.conyuge, indice.3, qx.hijo, estados.hijo,
   indice.4, edad, col, monto.suc.h, monto.suc.m, sucesion.aux.h, 
   sucesion.aux.m, n, indice, qx.pensionado, estados.pensionado)

### Activo actuarial

# Se crea una lista con las matrices calculadas
matrices.pension <- list(matriz.vejez.h, matriz.vejez.m, matriz.inv.h, matriz.inv.m, 
                         matriz.suc.h, matriz.suc.m, matriz.vejinv.h, matriz.vejinv.m,
                         matriz.suc.ch, matriz.suc.cm, matriz.suc.hh, matriz.suc.hm)

# Lista para matrices valor presente
matrices.vp <- list()

# se obtiene la matriz de forma mensualizada en valor presente
for (i in 1:length(matrices.pension)) {
  # Se extienden las matrices de forma mensual, considerando el aguinaldo
  matrices.vp[[i]] <- t(apply(matrices.pension[[i]], 1, function(fila) rep(fila, each = 13)))
  
  # Se obtiene el valor presente de cada matriz
  matrices.vp[[i]] <- matrices.vp[[i]] * matriz.vp[nrow(matrices.vp[[i]]), ]
}
...
# Se construye la matriz con los salarios totales para determinas la masa salarial
...
for(i in 1:ncol(salarios.finales)){
  ...
  costo.pens <- Reduce("+", sapply(matrices.pension, function(mat) sum(mat[, i])))
...
for(i in 1:ncol(salarios.finales)){
  ...
  costo.pens <- Reduce("+", lapply(matrices.pension, function(mat) sum(mat[, i])))
...
```{r grafico_annos_criticos_base, message = FALSE, warning = FALSE}
