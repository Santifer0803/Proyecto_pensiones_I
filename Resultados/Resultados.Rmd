...
# Se define la tasa de inflación para el escenario base
inflacion <- 0.04

# Se crea el vector con las tasas de acumulación por inflación
s <- ((1 + inflacion)^(0:(95 - 1)))
...
edades.completas <- matrix(rep(edades, 95), length(edades), ncol(edades.h))
colnames(edades.completas) <- colnames(edades.h)

mort.completa.h <- matrix(0, length(edades), ncol(edades.h))
mort.completa.m <- matrix(0, length(edades), ncol(edades.m))
...
# Vamos a crear las dos matrices aleatorias usando distribución Uniforme(0, 1)
...
A continuación se crean dos vectores para hombres y mujeres donde se identifica si al momento de la muerte el afiliado era mayor a 25 años (TRUE), lo que implica que tiene un hijo, y además que este hijo no tenga más de 25 años.
...
Ahora se calcula el beneficio por sucesión a quien corresponda.
...
restantes.ret.h <-
  apply(matriz.vejez.h, 1, function(fila) {
    which(fila == "Retirado")[1]
  })

restantes.ret.m <-
  apply(matriz.vejez.m, 1, function(fila) {
    which(fila == "Retirado")[1]
  })
...
filas <- Map(function(ini){
  
  fila <- numeric(max(restantes.ret.h) + length(s) - 1)
  
  fila[ini:(ini+length(s) - 1)] <- s
  
  fila
  
}, restantes.ret.h)

matriz.inflacion.h <- do.call(rbind, filas)[, 1:ncol(matriz.vejez.h)]

filas <- Map(function(ini){
  
  fila <- numeric(max(restantes.ret.m) + length(s) - 1)
...
matriz.inflacion.m <- do.call(rbind, filas)[, 1:ncol(matriz.vejez.m)]
...
for (i in 1:nrow(matriz.vejez.h)) {
  ...
  edad <- edades.vejez.h[i, indice]
  
  n <- min(nrow(mort.completa.m) - edad, ncol(mort.completa.m) - indice + 1)
  
  qx.conyuge <- mort.completa.m[cbind((edad + 1):(edad + n), 
                                      indice:(indice + n - 1))]
...
for (i in 1:nrow(matriz.vejez.m)) {
  ...
  qx.conyuge <- mort.completa.h[cbind((edad + 1):(edad + n), 
                                      indice:(indice + n - 1))]
...
monto.vejez.h <- monto.vejez.h * matriz.inflacion.h
...
monto.vejez.m <- monto.vejez.m * matriz.inflacion.m
...
filas <- Map(function(ini){
  ...
  fila <- numeric(max(restantes.inv.h) + length(s) - 1)
...
matriz.inflacion.h <- do.call(rbind, filas)[, 1:ncol(matriz.inv.h)]

filas <- Map(function(ini){
  
  fila <- numeric(max(restantes.inv.m) + length(s) - 1)
...
matriz.inflacion.m <- do.call(rbind, filas)[, 1:ncol(matriz.inv.m)]
...
for (i in 1:nrow(matriz.inv.h)) {
  ...
  matriz.inv.h[i, restantes.inv.h[i]:(indice - 1)] <- 1
  
  edad <- edades.inv.h[i, indice]
  ...
  if(edad >= 60){
    ...
    matriz.inv.h[i, indice:(indice + indice.4)] <- 
      matriz.inv.h[i, indice:(indice + indice.4)] + 0.3
...
monto.inv.h <- monto.inv.h * matriz.inflacion.h
...
monto.inv.m <- monto.inv.m * matriz.inflacion.m

matriz.inv.h <- matriz.inv.h * monto.inv.h
matriz.inv.m <- matriz.inv.m * monto.inv.m

colnames(matriz.inv.h) <- colnames(edades.h)
colnames(matriz.inv.m) <- colnames(edades.m)

rm(edades.inv.h, edades.inv.m, monto.inv.h, monto.inv.m)
```

### Pensiones por sucesión por sexo y año

```{r pensiones_sucesion}
...
filas <- Map(function(ini){
  ...
  fila <- numeric(max(col.fallecido.h) + length(s) - 1)
...
matriz.inflacion.h <- do.call(rbind, filas)[, 1:ncol(matriz.suc.h)]

filas <- Map(function(ini){
  
  fila <- numeric(max(col.fallecido.m) + length(s) - 1)
...
matriz.inflacion.m <- do.call(rbind, filas)[, 1:ncol(matriz.suc.m)]

# for hombres
for(i in 1:nrow(matriz.suc.h)){
  
  edad <- edad.fallecido.h[i]
  
  col <- col.fallecido.h[i]
...
mascara <- col(monto.suc.h) >= col.fallecido.h[row(monto.suc.h)]

monto.suc.h[mascara] <- sucesion.aux.h[row(monto.suc.h)[mascara]]

monto.suc.h <- monto.suc.h * matriz.inflacion.h
...
monto.suc.m <- monto.suc.m * matriz.inflacion.m

matriz.suc.h <- matriz.suc.h * monto.suc.h
matriz.suc.m <- matriz.suc.m * monto.suc.m

colnames(matriz.suc.h) <- colnames(edades.h)
colnames(matriz.suc.m) <- colnames(edades.m)

rm(qx.conyuge, vect.aleatorio, estados.conyuge, indice.3, qx.hijo, estados.hijo,
   indice.4, edad, col, mascara, monto.suc.h, monto.suc.m, matriz.inflacion.h, 
   matriz.inflacion.m, sucesion.aux.h, sucesion.aux.m, n, indice)
```
...
ben.dev.suc.h <-
  (cotizantes.activos$cotizaciones[which(cotizantes.activos$sexo == 1)][filas.muerte.h] / cuotas.finales.h[filas.muerte.h]) * (pension.sucesion.h) * tpx.sucesion.h * anualidades.sucesion.h * ((1 + interes.real) ^ -col.fallecido.h)
ben.dev.suc.m <-
  (cotizantes.activos$cotizaciones[which(cotizantes.activos$sexo == 2)][filas.muerte.m] / cuotas.finales.m[filas.muerte.m]) * (pension.sucesion.m) * tpx.sucesion.m * anualidades.sucesion.m * ((1 + interes.real) ^ -col.fallecido.m)
```
