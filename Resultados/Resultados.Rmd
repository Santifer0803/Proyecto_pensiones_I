...
# Poner las cotizaciones totales (determinista)
densidades_promedio_por_edad <- densidades_promedio_por_edad %>%
  mutate(densidades = round((
    densidades_promedio_por_edad$promedio_densidad / 100
  ) * 12, 0))

# Se quitan las edades mayores a 70
densidades_promedio_por_edad <- densidades_promedio_por_edad %>% 
  filter(edad < 71)

# Se agregan las edades restantes, que tienen densidad constante (determinista)
densidades_promedio_por_edad <- rbind(
  densidades_promedio_por_edad,
  data.frame(
    edad = 71:115,
    promedio_densidad = densidades_promedio_por_edad[nrow(densidades_promedio_por_edad), 2],
    densidad = densidades_promedio_por_edad[nrow(densidades_promedio_por_edad), 3]
  )
)

# Se transforma en DT (determinista)
cotizaciones.edad.dt <- setDT(densidades_promedio_por_edad)

# Se obtiene una matriz con la cantidad de cuotas por edad, a partir de 2025, separado por sexo (determinista)
matriz.cot.h <- matrix(
  cotizaciones.edad.dt[match(edades.h, edad), densidades],
  ncol = ncol(edades.h),
  nrow = nrow(edades.h),
  byrow = FALSE
)
matriz.cot.m <- matrix(
  cotizaciones.edad.dt[match(edades.m, edad), densidades],
  ncol = ncol(edades.m),
  nrow = nrow(edades.m),
  byrow = FALSE
)

# Matriz de 0, del tamaño de la predicción mensualizada (determinista)
...
inflacion.95 <- (inflacion_cr %>% filter(Año >= 1995))[, 3]
...
cotizaciones.vp <-
  as.matrix(cotizantes[, (4:ncol(cotizantes))]) * matrix(
    inflacion.95,
    nrow = nrow(cotizantes),
    ncol = ncol(cotizantes[(4:ncol(cotizantes))]),
    byrow = TRUE
  )

# Cotizantes separados por sexo (determinista)
...
cuotas.finales.h <-
  data_anual_sumado$cotizaciones[which(data_anual_sumado$sexo == 1)] + rowSums(cotizaciones.h)
cuotas.finales.m <-
  data_anual_sumado$cotizaciones[which(data_anual_sumado$sexo == 2)] + rowSums(cotizaciones.m)
...
salarios.top.h[, 1:12] <-
  salarios.top.h[, 1:12] * ifelse(((salarios.top.h[, 1:12] > 5000000) * ((salarios.top.h[, 1:12] - salarios.top.h[, 2:13]) > 500000)) == 1, 0, 1)
salarios.top.m[, 1:12] <-
  salarios.top.m[, 1:12] * ifelse(((salarios.top.m[, 1:12] > 5000000) * ((salarios.top.m[, 1:12] - salarios.top.m[, 2:13]) > 500000)) == 1, 0, 1)
...
beneficio.vejez.h <-
  (pmin(300, data_anual_sumado$cotizaciones[which(data_anual_sumado$sexo == 1)][which(salida.h == "Retirado")]) / cuotas.finales.h[which(salida.h == "Retirado")]) * (rowSums(salarios.top.h[which(salida.h == "Retirado"),]) / 300)
# Hacer la corrección a estados M, en donde las mujeres pueden retirarse a partir de los 63 si cumplen los requisitos
beneficio.vejez.m <-
  (pmin(300, data_anual_sumado$cotizaciones[which(data_anual_sumado$sexo == 2)][which(salida.m == "Retirado")]) / cuotas.finales.m[which(salida.m == "Retirado")]) * (rowSums(salarios.top.m[which(salida.m == "Retirado"),]) / 300)
...
requisito.cuotas.h <-
  ifelse(
    casillas.h == 1,
    rowSums(cotizaciones.vp[which(data_anual_sumado$sexo == 1),][filas.inv.h,][, 337:360] > 10000),
    ifelse(
      casillas.h == 2,
      rowSums(cotizaciones.vp[which(data_anual_sumado$sexo == 1),][filas.inv.h,][, 349:360] > 10000) + cotizaciones.h[filas.inv.h, 1],
...
requisito.cuotas.m <-
  ifelse(
    casillas.m == 1,
    rowSums(cotizaciones.vp[which(data_anual_sumado$sexo == 2),][filas.inv.m,][, 337:360] > 10000),
    ifelse(
      casillas.m == 2,
      rowSums(cotizaciones.vp[which(data_anual_sumado$sexo == 2),][filas.inv.m,][, 349:360] > 10000) + cotizaciones.m[filas.inv.m, 1],
...
beneficio.invalidez.h <-
  ifelse(cuotas.binario.h == 1,
         (rowSums(salarios.top.h[filas.inv.h, ][cuotas.binario.h == 1, ]) / 180),
         ifelse(requisito.cuotas.h == 1, ((
           cuotas.finales.h[filas.inv.h] / 180
         ) * (
           rowSums(salarios.top.h[filas.inv.h, ][requisito.cuotas.h == 1, ])
         )), 0))
beneficio.invalidez.m <-
  ifelse(cuotas.binario.m == 1,
         (rowSums(salarios.top.m[filas.inv.m, ][cuotas.binario.m == 1, ]) / 180),
         ifelse(requisito.cuotas.m == 1, ((
           cuotas.finales.m[filas.inv.m] / 180
         ) * (
           rowSums(salarios.top.m[filas.inv.m, ][requisito.cuotas.m == 1, ])
         )), 0))
```
