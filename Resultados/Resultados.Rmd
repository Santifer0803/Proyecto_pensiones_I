...
for (i in 1:length(matrices.pension)) {
  ...
  matrices.vp[[i]] <-
    matrices.vp[[i]][matrices.pension[[i]] > 2000000] * 0.95 + matrices.vp[[i]][matrices.pension[[i]] <= 2000000]
  matrices.pension[[i]] <-
    matrices.pension[[i]][matrices.pension[[i]] > 2000000] * 0.95 + matrices.pension[[i]][matrices.pension[[i]] <= 2000000]
}
...
Seguidamente, como pide el reglamento, se muestra el pasivo separado por generaciones y por razón de salida.

```{r pasivo_separado_base}
# Se defina una matriz para obtener los montos
matriz.pasivo <- matrix(0, nrow = 6, ncol = length(pensiones))

# Se obtienen los montos con un ciclo
for (i in 1:ncol(matriz.pasivo)) {
  matriz.pasivo[, i] <-
    c((sum(pensiones[[i]][[1]]) + sum(pensiones[[i]][[2]])),
      (sum(pensiones[[i]][[3]]) + sum(pensiones[[i]][[4]])),
      (sum(pensiones[[i]][[5]]) + sum(pensiones[[i]][[6]])),
      (sum(pensiones[[i]][[7]]) + sum(pensiones[[i]][[8]])),
      (sum(pensiones[[i]][[9]]) + sum(pensiones[[i]][[10]])),
      (sum(pensiones[[i]][[11]]) + sum(pensiones[[i]][[12]])))
}

# Se pone la información en un dataframe
(df.pasivo <-
  data.frame(
    "Generación actual" = c(rowSums(matriz.pasivo)[1:3], sum(rowSums(matriz.pasivo)[1:3])),
    "Generación pensionada" = c(rowSums(matriz.pasivo)[4:6], sum(rowSums(matriz.pasivo)[4:6])),
