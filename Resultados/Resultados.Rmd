...
pensionados$anio_nac <- year(pensionados$fec_nac) #extraer el anno de nacimiento
...
#se annaden columnas de relevancia
edades.pensionados.df <- as.data.frame(edades.pensionados)
rm(edades.pensionados)

sex.cod <- ifelse(pensionados$sexo == "M", 1, 2) # para que coincida con la codificacion de la matriz de mortalidad
...
for(i in 1:nrow(qx.pensionados.df)){ #itera sobre las filas (recorre cada pensionado especifico)
  
  sexo.esp <- qx.pensionados.df[i, 2] #la columna 2 es la del sexo, valor fijo para la iteracion i esima
  
  for (j in 3:ncol(qx.pensionados.df)) { #recorre cada columna (cada ano). empieza en 3 pq la 1 es de id y la 2 es de sexo
...
atipicos.h <- t(apply(diferencias.h, 1, function(fila){
  ...
  rachas.cortas <- with(v.bajos, rep(lengths < 12 & values, lengths))
...
# se calcula el porcentaje para pensión por viudez según edad del cónyuge
# por art. 27: 70% si cónyuge ≥ 60, 60% si 50–59, 50% si < 50
...
# asi como el porcentaje para pensión por orfandad (30% si el hijo califica)
# art. 27: 30% para hijos menores de 25 años
...
## verificar si tiene derecho al beneficio por sucesion (art. 18) 
...
# condición 2: haber cotizado al menos 12 veces en los últimos 24 meses antes de morir
# si murió antes de los 24 meses simulados, se usan los datos anteriores q estan en la base 
...
bono.cotizaciones <- 0.000833        # que excedan las 300 cotizaciones
bono.postergacion <- 0.001333         #por mes de postergacion
tasa.maxima <- 1.25

salario.minimo <- 12236.95 * 22  #tomando como supuesto 4 fines de semana al mes, y meses de 30 dias, se multiplica el salario por jornada diaria ordinaria por los dias laborales del mes
