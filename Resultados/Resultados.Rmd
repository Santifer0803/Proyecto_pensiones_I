...
edad.final.h <- restantes.inv.h[filas.inv.h] + edades.h[filas.inv.h, 1] - 1
edad.final.m <- restantes.inv.m[filas.inv.m] + edades.m[filas.inv.m, 1] - 1
...
col.fallecido.h <- apply(estados.h[filas.muerte.h, ], 1, function(fila) {
  which(fila == "Fallecido")[1]
})
col.fallecido.m <- apply(estados.m[filas.muerte.m, ], 1, function(fila) {
  which(fila == "Fallecido")[1]
})

# Edad de la persona al morir
edad.fallecido.h <- mapply(function(i, j) edades.h[i, j], i = filas.muerte.h, j = col.fallecido.h)
edad.fallecido.m <- mapply(function(i, j) edades.m[i, j], i = filas.muerte.m, j = col.fallecido.m)
```
...
beneficio.base.h <- rowSums(salarios.sucesion.h) / 300
beneficio.base.m <- rowSums(salarios.sucesion.m) / 300

# Se calcula el porcentaje para pensión por viudez según edad del cónyuge por art. 27: 70% si cónyuge ≥ 60, 60% si 50–59, 50% si < 50
porcentaje.viudez.h <- ifelse(edad.fallecido.h >= 60, 0.70,
                              ifelse(edad.fallecido.h >= 50, 0.60, 0.50))
porcentaje.viudez.m <- ifelse(edad.fallecido.m >= 60, 0.70,
...
requisito.24meses.h <- mapply(function(i, j) {
  if (j <= 2) {
    cot.h <- cotizaciones.vp[which(cotizantes.activos$sexo == 1), ]
    sum(cot.h[i, 337:360] > 10000, na.rm = TRUE) >= 12
  } else {
    sum(cotizaciones.h[i, (j - 2):(j - 1)], na.rm = TRUE) >= 12
  }
}, i = filas.muerte.h, j = col.fallecido.h)

requisito.24meses.m <- mapply(function(i, j) {
  if (j <= 2) {
    cot.m <- cotizaciones.vp[which(cotizantes.activos$sexo == 2), ]
    sum(cot.m[i, 337:360] > 10000, na.rm = TRUE) >= 12
  } else {
    sum(cotizaciones.m[i, (j - 2):(j - 1)], na.rm = TRUE) >= 12
  }
}, i = filas.muerte.m, j = col.fallecido.m)
...
beneficio.conyuge.h <- ifelse(tiene.derecho.sucesion.h, beneficio.conyuge.h, 0)
beneficio.hijo.h    <- ifelse(tiene.derecho.sucesion.h, beneficio.hijo.h, 0)

beneficio.conyuge.m <- ifelse(tiene.derecho.sucesion.m, beneficio.conyuge.m, 0)
beneficio.hijo.m    <- ifelse(tiene.derecho.sucesion.m, beneficio.hijo.m, 0)
```
...
salario.referencia.h <- beneficio.vejez.h[filas.vejez.h]
salario.referencia.m <- beneficio.vejez.m[filas.vejez.m]
...
meses.post.h <- rowSums(postergacion.simulada.h[filas.vejez.h, ])
meses.post.m <- rowSums(postergacion.simulada.m[filas.vejez.m, ])
...
# Verificacion del límite por reglamentario
tasa.vejez.h <- pmin(tasa.vejez.h, tasa.maxima)
tasa.vejez.m <- pmin(tasa.vejez.m, tasa.maxima)

# Pensión final por vejez
pension.vejez.h <- salario.referencia.h * tasa.vejez.h
pension.vejez.m <- salario.referencia.m * tasa.vejez.m
```
...
# El salario base
salario.referencia.inv.h <- beneficio.invalidez.h[filas.inv.h]
salario.referencia.inv.m <- beneficio.invalidez.m[filas.inv.m]
...
# Verificacion del límite
tasa.invalidez.h <- pmin(tasa.invalidez.h, tasa.maxima)
tasa.invalidez.m <- pmin(tasa.invalidez.m, tasa.maxima)

# La pensión por invalidez
pension.invalidez.h <- salario.referencia.inv.h * tasa.invalidez.h
pension.invalidez.m <- salario.referencia.inv.m * tasa.invalidez.m
```
